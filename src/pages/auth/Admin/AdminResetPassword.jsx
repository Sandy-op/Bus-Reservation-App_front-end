...
//     try {
//       const response = await axios.put(`http://deepmindstech.in:8080/api/admins/reset-password/${email}`, {
...
const AdminResetPassword = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_URL}/api/admins/reset-password`, {
        token,
        password: newPassword,
      });
      ...
      if (response.status === 202) {
        ...
        setTimeout(() => navigate("/adminAuth"), 3000);
      } else {
